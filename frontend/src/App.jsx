...
const SalaryPrediction = () => {
  ...
  const API_URL =
    "https://employee-salary-prediction-5lz69kfy2-sathiyaseelan-ss-projects.vercel.app";
  useEffect(() => {
    const fetchOptionsFromCSV = async () => {
      try {
        ...
        Papa.parse(csvText, {
          ...
          complete: (results) => {
            const companies = [
              ...new Set(results.data.map((item) => item.companies)),
            ];
            const jobTypes = [
              ...new Set(results.data.map((item) => item.jobTypes)),
            ];
            const degrees = [
              ...new Set(results.data.map((item) => item.degrees)),
            ];
            const majors = [
              ...new Set(results.data.map((item) => item.majors)),
            ];
            const industries = [
              ...new Set(results.data.map((item) => item.industries)),
            ];
